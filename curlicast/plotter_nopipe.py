...
def read_cells_from_sacc(sacc_file, type, tracer_list, lmin, lmax,
    ...
    for tr1, tr2 in cwr(tracer_list, 2):
        if return_cov:
            ls, cl, cov = s.get_ell_cl(type, tr1, tr2, return_cov=True)
        else:
            ls, cl = s.get_ell_cl(type, tr1, tr2)
        msk = np.logical_and(ls <= lmax, ls >= lmin)
        ls = ls[msk]
...
class Plotter(object):
    """
    Plotting stage for Curlicast. Plots power spectra, best-fit models, and
    posterior chains.
    """
    def __init__(self, args):
        ...
        self.config = config["global"] | config["Plotter"]
    ...
    def create_page(self):
        ...
        # Create HTML page
        self.doc = dom.document(title='Curlicast plots page')
        with self.doc.head:
            ...
            for t1, t2 in self.s_cd_x.get_tracer_combinations():
                for p1, p2 in cwr(self.pols, 2):
                    ...
                    if do_best:
                        ...
                        simple_chi2 = np.sum((y - cl_best[msk_best])**2/yerr**2)  # noqa
                        ndof = len(y)
                        main.plot(
                            [], [], ls="",
    ...
    def write_page(self):
        """
        """
        plots_page = self.config['plots_page']
    ...
    def read_inputs(self):
        ...
        # Chains
        chain_file = self.config["chain_file"]
        ...
        # Best-fit parameters
        chi2_file = self.config["chi2_file"]
...
def main(args):
    """
    Execute the Plotter stage with arguments args:
        * config: string.
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Plotting stage for Curlicast"
    )
    parser.add_argument(
        "--config", type=str,
